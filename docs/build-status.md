...
- **3.4 Office preview:** Source `.pptx` / `.ppt`, `.docx` / `.doc`, and `.xlsx` / `.xls` files render inline by converting through LibreOffice (`soffice`) to a cached PDF under `.aiwiki/cache/source-pdf/<stem>-<extension>-<mtime>.pdf`, then reusing the existing PDF.js viewer. Cache keys include source mtime so changed files invalidate automatically.
